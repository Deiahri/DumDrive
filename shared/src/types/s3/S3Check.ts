import { ForbiddenS3BucketPrefixes, GetDirResponse, S3File, S3Folder } from "./S3Types";
...
export function checkGetDirResponse(obj: any): asserts obj is GetDirResponse {
  ...
  const { files, folders, path, nextContinuationToken, success } = obj as GetDirResponse;
  ...
  if (nextContinuationToken !== undefined && typeof nextContinuationToken !== 'string') {
    throw new Error("Invalid GetDirResponse: 'nextContinuationToken' must be a string.");
  }
 ...
 * - "Invalid bucket name: contains illegal characters or dots." if the bucket name contains characters other than lowercase letters, numbers, or hyphens, or if it contains dots.
 * 
 * @remarks
 * - The function uses a regular expression (`/^[a-z0-9-]+$/`) to validate the bucket name.
 * - Dots (`.`) are disallowed to avoid the complexities and restrictions associated with S3 bucket names containing dots, such as issues with SSL/TLS certificates and region-specific endpoints.
 * - This function is designed to enforce a simplified subset of S3 bucket naming rules for easier management.
 */
export function checkBucketName(obj: string, makeForbiddenException?: boolean): boolean {
  if (!makeForbiddenException && ForbiddenS3BucketPrefixes.some(prefix => obj.startsWith(prefix))) {
    throw new Error(`Invalid bucket name: must not start with any of the forbidden prefixes (${ForbiddenS3BucketPrefixes.join(', ')}).`);
  }
  ...
  if (!bucketNameRegex.test(obj)) {
    throw new Error("Invalid bucket name: contains illegal characters or dots.");
  }
  return true;
}
